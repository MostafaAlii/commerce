                ...
                <form action="{{route('offers.update', $offer->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col">
                            ...
                            <input id="name" type="text" name="name" class="form-control" value="{{ $offer?->name }}">
                        </div>
                        <div class="col">
                            <label for="is_active" class="mr-sm-2">الحاله :</label>
                            <div class="box">
                                <select class="form-control" name="status">
                                    <option value="1" {{ (old('status', $offer->status) == '1') ? 'selected' : '' }} >فعال</option>
                                    <option value="0" {{ (old('status', $offer->status) == '0') ? 'selected' : '' }} >غير فعال</option>
                                </select>
